...
namespace xbWatson
{
    public partial class xbWatsonUI : Form
    {
        ...
        private void xbWatsonUI_FormClosing(object sender, FormClosingEventArgs e)
        {
        }
        ...
        {
            // Save preference
            using (var regKey = Registry.CurrentUser.CreateSubKey("Software\\Microsoft\\XenonSDK\\xbWatson\\Options"))
            {
                regKey?.SetValue("DarkMode", enable ? "True" : "False");
            }
            // Only update child xbWatson windows
            foreach (var child in this.MdiChildren)
            {
                if (child is xbWatson watson)
                {
                    watson.SetDarkMode(enable);
                }
            }
        }
        ...
        {
            base.OnLoad(e);
            using (var regKey = Registry.CurrentUser.OpenSubKey("Software\\Microsoft\\XenonSDK\\xbWatson\\Options"))
            {
                var darkPref = regKey?.GetValue("DarkMode") as string;
                isDarkMode = darkPref == "True";
            }
            SetDarkMode(isDarkMode);
            // Set menu text and tooltip to match mode
            darkModeToolStripMenuItem.Text = isDarkMode ? "Light Mode" : "Dark Mode";
            darkModeToolStripMenuItem.ToolTipText = isDarkMode ? "Switch to light mode" : "Switch to dark mode";
